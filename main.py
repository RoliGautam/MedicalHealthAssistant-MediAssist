...
def helper(dis):
    try:
        ...
        med = medications[medications['Disease'] == dis]['Medication'].apply(eval).tolist()
        med = [item for sublist in med for item in sublist]  # Flatten the list

        die = diets[diets['Disease'] == dis]['Diet'].apply(eval).tolist()
        die = [item for sublist in die for item in sublist]  # Flatten the list
...
def get_predict():
    try:
        # Extract symptoms from query parameters
        ...
        response = {
            ...
            "medications": medications,
            "diet": rec_diet,
            "workout": workout.tolist() if hasattr(workout, 'tolist') else workout
        }
